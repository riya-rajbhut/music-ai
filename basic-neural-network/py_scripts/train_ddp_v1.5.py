...
def main_worker(gpu, world_size, hparams):
    ...
    for epoch in range(hparams['epochs']):
        ...
        for inputs, targets in train_loader:
            ...
            with torch.no_grad():
                predicted = torch.argmax(preds['pitch'], dim=1)
                correct_train += (predicted == y_pitch).sum().item()
                total_train += y_pitch.size(0)
        ...
        with torch.no_grad():
            for inputs, targets in val_loader:
                ...
                with autocast('cuda'):
                    ...
                    predicted = torch.argmax(preds['pitch'], dim=1)
                    correct_val += (predicted == y_pitch).sum().item()
                    total_val += y_pitch.size(0)
        ...
        metrics = torch.tensor([
            ...
            correct_val,
            total_val,
            running_grad_norm / len(train_loader)
        ], device=gpu)
        dist.all_reduce(metrics, op=dist.ReduceOp.SUM)

        train_l = (metrics[0] / world_size).item()
        train_p_l = (metrics[1] / world_size).item()
        val_l = (metrics[2] / world_size).item()
        val_p_l = (metrics[3] / world_size).item()
        train_acc = (metrics[4] / metrics[5]).item()
        val_acc = (metrics[6] / metrics[7]).item()
        grad_flow = (metrics[8] / world_size).item()
        ...
        if is_main_process:
            ...
            if (epoch + 1) % 10 == 0:
                weights = model.module.pitch_head[-1].weight.detach().cpu().numpy().flatten()
