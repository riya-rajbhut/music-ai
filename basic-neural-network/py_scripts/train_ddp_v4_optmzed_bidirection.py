...
class OptimizedMusicRNN(nn.Module):
    """Predicts next pitch using pitch class and octave decomposition (BiLSTM)."""
    def __init__(self, num_pitches=128, pitch_embed_dim=128, hidden_size=384, num_layers=3, dropout_rate=0.3):
        ...
        # 1. Enable bidirectional processing
        self.lstm = nn.LSTM(
        ...
        # 2. Double the input dimensions to account for concatenated forward/backward states
        self.skip_proj = nn.Linear(pitch_embed_dim, hidden_size * 2)
        self.gate_proj = nn.Linear(hidden_size * 2, hidden_size * 2)

        # 3. Project the doubled hidden state back down to the standard hidden_size
    ...
    def forward(self, pitch_seq):
        ...
        x = self.input_norm(pitch_embeds)

        # h_n shape: (num_layers * 2, batch_size, hidden_size)
        _, (h_n, _) = self.lstm(x)
        
        # 4. Concatenate the final forward state (h_n[-2]) and final backward state (h_n[-1])
