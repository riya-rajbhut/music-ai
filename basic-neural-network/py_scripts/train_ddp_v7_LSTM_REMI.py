...
class Seq2SeqMusicLSTM(nn.Module):
    """Predicts next REMI tokens in sequence using an LSTM."""
    def __init__(self, num_tokens=384, hidden_size=768, num_layers=3, dropout_rate=0.1):
        super().__init__()
        ...
        # The core LSTM Sequence-to-Sequence block
        self.lstm = nn.LSTM(
    ...
    def forward(self, token_seq):
        # Call this before passing data into the LSTM to optimize memory for cuDNN!
        self.lstm.flatten_parameters()
        
        # Embed the integer tokens into dense vectors
        x = self.token_embed(token_seq)
        
        # Pass sequentially through the LSTM (_ ignores the unused hidden and cell states)
        lstm_out, _ = self.lstm(x)
        
        # Map the LSTM outputs back to the vocabulary probabilities
